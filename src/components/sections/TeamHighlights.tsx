  ...
  useEffect(() => {
    const fetchTeamMembers = async () => {
      try {
        ...
        // Show only first 4 active members for highlights
        setTeamMembers(members.filter(member => member.is_active !== false).slice(0, 4));
      } catch (err) {
        console.error('Error fetching team members:', handleApiError(err));
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {loading ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
              {teamMembers.map((member, index) => (
                <SlideUpSection key={member.id} delay={index * 0.1}>
                  ...
                  >
                    <Card className="h-full p-6 text-center group">
                      {/* Profile Image */}
            ...
            <div className="text-center py-16 mb-16">
              ...
              <h3 className="text-xl font-semibold text-gray-600 mb-2">Team Information Coming Soon</h3>
              <p className="text-gray-500">We're building an amazing team to transform Africa through AI.</p>
            </div>
          )}
